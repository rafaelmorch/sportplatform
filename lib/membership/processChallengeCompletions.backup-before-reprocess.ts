...
export async function processChallengeCompletions({
  ...
  for (const challenge of activeChallenges) {
    ...
    let individualActivitiesQuery = supabase
      .from("strava_activities")
      .select("activity_id, distance, moving_time, elapsed_time, type, sport_type, start_date, start_date_local")
      .eq("athlete_id", athleteId);

    if (!includeAllHistory && evaluationStartIso) {
      individualActivitiesQuery = individualActivitiesQuery.gte(
        "start_date",
        evaluationStartIso
      );
    }
    ...
    } = await individualActivitiesQuery;
