...
if (!proofFile) {
  setErrorText("Please select a proof photo.");
  setLoading(false);
  return;
}
...
proofPath = `${communityId}/${challengeId}/${user.id}-${Date.now()}.${extension}`;
...
{previewUrl && (
  ...
  />
          ...
          >
            {loading || uploading ? "Uploading..." : "Submit proof"}
          </button>
        </section>
      </div>
    </main>
