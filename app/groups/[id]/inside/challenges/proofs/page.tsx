...
export default function ChallengeProofsAdminPage() {
  ...
  async function handleApprove(proof: ProofRow) {
    ...
    const { data: challengeData, error: challengeError } = await supabase
      ...
      .select("id, title, activity_type, deadline, points_active, points_late, runner_level")
    ...
    const { error: checkinError } = await supabase
      ...
      .insert({
        ...
        show_in_feed: proof.share_to_feed ?? true,
        points,
    ...
    const completedLevel = challengeData.runner_level || "yellow";
    const currentLevelIndex = levelOrder.indexOf(completedLevel);
    const nextLevel = currentLevelIndex >= 0 ? levelOrder[currentLevelIndex + 1] : null;

    if (nextLevel) {
      const { data: levelChallenges } = await supabase
        .from("app_membership_challenges")
        .select("id, runner_level")
        .eq("community_id", proof.community_id)
        .eq("is_active", true);

      const challengesInLevel = ((levelChallenges || []) as Array<{ id: string; runner_level: string | null }>).filter(
        (challenge) => (challenge.runner_level || "yellow") === completedLevel
      );

      const { data: userCheckins } = await supabase
        .from("app_membership_checkins")
        .select("challenge_id, is_disregarded")
        .eq("community_id", proof.community_id)
        .eq("user_id", proof.user_id);

      const completedIds = new Set(
        ((userCheckins || []) as Array<{ challenge_id: string | null; is_disregarded: boolean | null }>)
          .filter((checkin) => !checkin.is_disregarded && checkin.challenge_id)
          .map((checkin) => checkin.challenge_id as string)
      );

      const completedAllLevelChallenges =
        challengesInLevel.length > 0 &&
