...
export default function EventDetailPage() {
  ...
  // ✅ PAGO: abre checkout em NOVA ABA
  async function handleRegister() {
    ...
    try {
      ...
      // Evento pago: chama API e abre URL
      if (price > 0) {
        ...
        // tentativa de nova aba (depende do browser)
        const win = window.open(url, "_blank", "noopener,noreferrer");
        if (!win) {
          // fallback: mesma aba (pra não travar)
          window.location.href = url;
        }
        return;
      }

      // Evento grátis: insert direto (RLS precisa permitir)
