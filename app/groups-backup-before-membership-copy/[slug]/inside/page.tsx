...
export default function MembershipInsidePage() {
  ...
  useEffect(() => {
    async function checkAccessAndLoad() {
      ...
      const { data: community } = await supabase
        .from("app_membership_communities")
        .select("id, name, created_by, slug").eq("slug", id)
        .single();
...
const typedCommunity = community as CommunityRow;
      ...
      setCommunityId(typedCommunity.id);
      setCommunityName(typedCommunity.name || null);
      setCanManageHighlights(canManageCommunity);
      setHighlights(visibleHighlights);

      await Promise.all([
        loadFeed(typedCommunity.id, user.id),
        loadCheckins(typedCommunity.id, user.id, canManageCommunity),
        loadRanking(typedCommunity.id, typedCommunity.created_by ?? null, user.id),
        loadLeaderOfMonth(typedCommunity.id),
        loadChallenges(typedCommunity.id),
        loadVideosFlag(typedCommunity.id),
      ]);
