...
function getNext7DaySuggestions(params) {
  ...
  if (goal.includes("ironman") || goal.includes("triathlon")) {
    suggestions.push("Corrida leve de 40 min em ritmo confortável");
    suggestions.push("Bike de 60 min em zona aeróbica");
    suggestions.push("Treino técnico de natação");
    suggestions.push("Treino de força (core + estabilidade)");
    suggestions.push("Brick: bike + corrida curta");
    suggestions.push("Mobilidade e recuperação ativa");
    suggestions.push("Longo leve no final da semana");
  } else {
    suggestions.push("Treino leve de 30–40 min");
    suggestions.push("Treino moderado contínuo");
    suggestions.push("Treino de força geral");
    suggestions.push("Recuperação ativa");
    suggestions.push("Intervalado leve");
    suggestions.push("Mobilidade");
    suggestions.push("Treino contínuo confortável");
  }

  if (weeklyActivitiesCount >= 5) {
    suggestions[0] = "Recuperação ativa — você já treinou bastante";
  }

  if (weeklyDistanceKm > 30) {
    suggestions[1] = "Reduzir intensidade hoje para evitar sobrecarga";
  }
    ...
    const loadPage = async () => {
      ...
      if (tokenRow?.athlete_id) {
        ...
        setStravaActivities((activitiesData ?? []) as StravaActivityRow[]);
            ...
            const { data: planningProfile } = await supabase
        .from("performance_ai_profiles")
        .select("goal_text, goal_date, level, days_per_week, minutes_per_session, sports")
        .eq("user_id", user.id)
        .maybeSingle();

      if (planningProfile) {
        setGoalText(planningProfile.goal_text ?? "");
        setGoalDate(planningProfile.goal_date ?? "");
        setLevel(planningProfile.level ?? "");
        setDaysPerWeek(planningProfile.days_per_week ?? null);
        setMinutesPerSession(planningProfile.minutes_per_session ?? null);
        setSports(planningProfile.sports ?? "");
      }
